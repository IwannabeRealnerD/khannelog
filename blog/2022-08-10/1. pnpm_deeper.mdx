...
> symlink, hardlink를 이용하여 효율적인 디스크 공간 관리 및 속도를 중요시 하는 패키지 관리자
...
- index-node의 줄임말
- 파일 시스템에 `주민등록번호` -> 파일 시스템 내에 파일, 디렉터리들은 고유한 inode값을 가지고 있음
...
- 디렉터리와 디렉터리는 hardlink 불가 -> symlink는 가능
...
ls -a # react, js-tokens@4.0.0, loose-envify@1.4.0, react@18.2.0, lock.yaml, node_modules
```
